...
{
	SizeUpX(m_pInfoInstance->Get_SpellLearn(Index).fSpellSize);
	m_pSpell_Name = m_pInfoInstance->Get_SpellLearn(Index).pSpellName;
}
...
HRESULT CSpellLearn_Name::Ready_Components(void* pArg)
{
	...
	if (FAILED(Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Texture_UI_T_Spell_Name"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
