...
CGoblin_ProtegoHit::CGoblin_ProtegoHit(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Container{ pDevice, pContext }
{
}

CGoblin_ProtegoHit::CGoblin_ProtegoHit(const CGoblin_ProtegoHit& rhs)
	: CEffect_Container(rhs)
{
}

HRESULT CGoblin_ProtegoHit::Initialize_Prototype()
{
	if (FAILED(Load_Package("../Bin/Resources/Data/Effect/MonsterPackage/GoblinProtegoHit")))
		return E_FAIL;
...
HRESULT CGoblin_ProtegoHit::Initialize(void* pArg)
{
	...
	m_wstrEffectName = L"GoblinProtegoHit";
...
void CGoblin_ProtegoHit::Priority_Update(_float fTimeDelta)
{
	__super::Priority_Update(fTimeDelta);

}

void CGoblin_ProtegoHit::Update(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
void CGoblin_ProtegoHit::Late_Update(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
HRESULT CGoblin_ProtegoHit::Pre_Setting(CGameObject* pObject, void* pArg)
{
...
HRESULT CGoblin_ProtegoHit::Ready_Components(void* pArg)
{
...
HRESULT CGoblin_ProtegoHit::Ready_Child()
{
	return S_OK;
}

CGoblin_ProtegoHit* CGoblin_ProtegoHit::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CGoblin_ProtegoHit* pInstance = new CGoblin_ProtegoHit(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CGoblin_ProtegoHit");
...
CGameObject* CGoblin_ProtegoHit::Clone(void* pArg, CGameObject* pOwner)
{
	CGoblin_ProtegoHit* pInstance = new CGoblin_ProtegoHit(*this);
	pInstance->m_pOwner = pOwner;
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CGoblin_ProtegoHit");
		SAFE_RELEASE(pInstance);
	}
...
void CGoblin_ProtegoHit::OnCollision(CGameObject* pOther, void* pDesc)
{
...
void CGoblin_ProtegoHit::Describe_Entity()
{
...
HRESULT CGoblin_ProtegoHit::Bind_ShaderResources()
{
	return S_OK;
}
