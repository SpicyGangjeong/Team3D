...
CBroom_Scoreboard::CBroom_Scoreboard(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CElementObject(pDevice, pContext)
{
}

CBroom_Scoreboard::CBroom_Scoreboard(const CBroom_Scoreboard& rhs)
	:CElementObject(rhs)
{
}

HRESULT CBroom_Scoreboard::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBroom_Scoreboard::Initialize(void* pArg)
{
...
void CBroom_Scoreboard::Priority_Update(_float fTimeDelta)
{
...
void CBroom_Scoreboard::Update(_float fTimeDelta)
{
	if (!__super::Chack_Visible())
	{
		return;
...
void CBroom_Scoreboard::Late_Update(_float fTimeDelta)
{
	if (!__super::Chack_Visible())
	{
		return;
...
HRESULT CBroom_Scoreboard::Render()
{
...
void CBroom_Scoreboard::Set_Setting()
{
	...
	Score[4] = TEXT("0");
	Score[5] = TEXT("29");
	Score[6] = TEXT("0:00:00");
	Score[7] = TEXT("0:00:00");

	m_fFontPosition[0] = _float2(m_fX + 770, m_fY - 245.f);
	m_fFontPosition[1] = _float2(m_fX + 1050, m_fY - 245.f);
	m_fFontPosition[2] = _float2(m_fX + 918, m_fY - 195.f);
...
void CBroom_Scoreboard::Set_MaxRing(_int iMaxRing)
{
	Score[5] = to_wstring(iMaxRing);
}

void CBroom_Scoreboard::Set_CurrentRing()
{
	m_iCurrentRing++;
...
HRESULT CBroom_Scoreboard::Bind_ShaderResources()
{
...
HRESULT CBroom_Scoreboard::Ready_Components(void* pArg)
{
	if (FAILED(Add_Component<CVIBuffer_Rect>(g_iStaticLevel, &m_pVIBufferCom)))
	{
		return E_FAIL;
	}
	if (FAILED(Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Texture_UI_T_Scoreboard"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
...
CBroom_Scoreboard* CBroom_Scoreboard::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBroom_Scoreboard* pInstance = new CBroom_Scoreboard(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CBroom_Scoreboard");
...
CGameObject* CBroom_Scoreboard::Clone(void* pArg, CGameObject* pOwner)
{
	CBroom_Scoreboard* pInstance = new CBroom_Scoreboard(*this);
	pInstance->m_pOwner = pOwner;
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBroom_Scoreboard");
		SAFE_RELEASE(pInstance);
	}
...
#ifdef _DEBUG
void CBroom_Scoreboard::Describe_Entity()
{
}
#endif // _DEBUG
