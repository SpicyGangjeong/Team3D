...
HRESULT CRanrok_Swipe::Initialize(void* pArg)
{
	...
	m_pRock_PT = Get_PartObject<CEffectParts>("Rock_PT");
	...
	m_Events.emplace(0.4f, [&]() {
		...
		CEffectParts* pClawDecal = Get_PartObject<CEffectParts>("ClawDecal");
		pClawDecal->Set_Visible(true);

		CEffectParts* pSmoke = Get_PartObject<CEffectParts>("Swipe_Smoke");
