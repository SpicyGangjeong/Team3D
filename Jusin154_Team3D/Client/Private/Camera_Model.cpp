...
void CCamera_Model::Late_Update(_float fTimeDelta)
{
	...
	auto M = m_pModelCom->Get_BoneMatrix("lookat_target");
	GUI::Text("Bone T: %.3f %.3f %.3f", M.r[3].m128_f32[0], M.r[3].m128_f32[1], M.r[3].m128_f32[2]);


	_vector vTargetPos = m_pModelCom->Get_BoneMatrix("lookat_target").r[3];

	m_pTransformCom->LookAt(
		XMVectorSetW(vTargetPos, 1.f)
	);
...
HRESULT CCamera_Model::Ready_Components(void* pArg)
{
	...
	if (FAILED(__super::Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Component_CCL_CameraRig.001_Model"),
