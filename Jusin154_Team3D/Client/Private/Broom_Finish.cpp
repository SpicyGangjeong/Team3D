...
HRESULT CBroom_Finish::Render()
{
	...
	m_pGameInstance->Render_Text(TEXT("Font_size20"), m_fFont[1].c_str(), m_fFontPos[1], XMVectorSet(0.f, 1.f, 0.f, 1.f));
...
void CBroom_Finish::Finish(_float fTime)
{
	wchar_t szBuf[32] = {};
...
HRESULT CBroom_Finish::Ready_Components(void* pArg)
{
	...
	if (FAILED(Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Texture_UI_T_Finish"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
