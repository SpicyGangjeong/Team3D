...
CSpellLearn_Overlay::CSpellLearn_Overlay(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CElementObject(pDevice, pContext)
{
}

CSpellLearn_Overlay::CSpellLearn_Overlay(const CSpellLearn_Overlay& rhs)
	:CElementObject(rhs),
	m_pInfoInstance(CInfoInstance::GetInstance())
{
}

HRESULT CSpellLearn_Overlay::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CSpellLearn_Overlay::Initialize(void* pArg)
{
...
void CSpellLearn_Overlay::Set_SpellLearn(_int Index)
{
	Compute_UI(m_pInfoInstance->Get_SpellLearn(Index).iSpellID);
	m_vUVScale.y = 0.f;
	m_fTrail = 0.f;
	m_iSpellType = m_pInfoInstance->Get_SpellLearn(Index).iSpellType;
	m_fSpellLearn = _float(m_pInfoInstance->Get_SpellLearn(Index).Lines.size());
	MoveX(m_pInfoInstance->Get_SpellLearn(Index).fEndPosition.x);
	MoveY(m_pInfoInstance->Get_SpellLearn(Index).fEndPosition.y);
	SizeUpdate(m_fOrigin_Size.x, m_fOrigin_Size.y);
	m_fSizeUp = false;
	m_bClear = false;
}

void CSpellLearn_Overlay::Set_Count(_int Index)
{
	m_fTrail = _float(Index);
}

void CSpellLearn_Overlay::Priority_Update(_float fTimeDelta)
{
...
void CSpellLearn_Overlay::Update(_float fTimeDelta)
{
	if (!__super::Chack_Visible())
	{
		return;
...
void CSpellLearn_Overlay::Late_Update(_float fTimeDelta)
{
	if (!__super::Chack_Visible())
	{
		return;
...
HRESULT CSpellLearn_Overlay::Render()
{
...
_vector CSpellLearn_Overlay::Get_WorldPostion()
{
	return m_pTransformCom->Get_State(STATE::POSITION);
}

HRESULT CSpellLearn_Overlay::Bind_ShaderResources()
{
...
HRESULT CSpellLearn_Overlay::Ready_Components(void* pArg)
{
...
void CSpellLearn_Overlay::Compute_UI(_uint SpellID)
{
...
void CSpellLearn_Overlay::SizeUp(_float fSizeX)
{
	...
	if (160.f <= m_fSizeX + fSizeX)
	{
		...
		m_bClear = false;
		m_fSizeUp = true;
		static_cast<CUIObject*>(m_pOwner)->Function_Callback(TEXT("OverrlayClear"));
	}
}

CSpellLearn_Overlay* CSpellLearn_Overlay::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CSpellLearn_Overlay* pInstance = new CSpellLearn_Overlay(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CSpellLearn_Overlay");
...
CGameObject* CSpellLearn_Overlay::Clone(void* pArg, CGameObject* pOwner)
{
	CSpellLearn_Overlay* pInstance = new CSpellLearn_Overlay(*this);
...
#ifdef _DEBUG
void CSpellLearn_Overlay::Describe_Entity()
{
}
#endif // _DEBUG
