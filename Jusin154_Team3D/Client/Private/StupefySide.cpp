...
CStupefySide::CStupefySide(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Container{ pDevice, pContext }
{
}

CStupefySide::CStupefySide(const CStupefySide& rhs)
	: CEffect_Container(rhs)
{
}

HRESULT CStupefySide::Initialize_Prototype()
{

	if (FAILED(Load_Package("../Bin/Resources/Data/Effect/Package/StupefySide")))
		return E_FAIL;
...
HRESULT CStupefySide::Initialize(void* pArg)
{
	...
	m_pWandParticle = Get_PartObject<CEffectParts>("Wand_Particle_B");
...
void CStupefySide::Priority_Update(_float fTimeDelta)
{
	__super::Priority_Update(fTimeDelta);

}

void CStupefySide::Update(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
void CStupefySide::Late_Update(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
HRESULT CStupefySide::Pre_Setting(CGameObject* pObject, void* pArg)
{
...
HRESULT CStupefySide::Ready_Components(void* pArg)
{
...
HRESULT CStupefySide::Ready_Child()
{
	return S_OK;
}

CStupefySide* CStupefySide::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CStupefySide* pInstance = new CStupefySide(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CStupefySide");
...
CGameObject* CStupefySide::Clone(void* pArg, CGameObject* pOwner)
{
	CStupefySide* pInstance = new CStupefySide(*this);
	pInstance->m_pOwner = pOwner;
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CStupefySide");
		SAFE_RELEASE(pInstance);
	}
...
void CStupefySide::OnCollision(CGameObject* pOther, void* pDesc)
{
	ON_COLLISION_INFO* CollisionDesc = static_cast<ON_COLLISION_INFO*>(pDesc);

}

void CStupefySide::Free()
{
	__super::Free();
...
void CStupefySide::Describe_Entity()
{
...
HRESULT CStupefySide::Bind_ShaderResources()
{
	return S_OK;
}
