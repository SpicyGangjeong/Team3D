...
void CDialogue_Choice::Up()
{
	m_fY += 70.f;
}

void CDialogue_Choice::Text(_wstring pText)
{
	m_pText = pText;
}
...
void CDialogue_Choice::Update(_float fTimeDelta)
{
	...
	if (m_bHover == true)
	{
		...
		{
			static_cast<CUIObject*>(m_pOwner)->Function_Callback(TEXT("TEXTTYPECHOICE"), &m_ChoiceInfo);
		}
	}
...
HRESULT CDialogue_Choice::Render()
{
	...
	m_pGameInstance->Render_Text(TEXT("Font_size20"), m_pText.c_str(), _float2(m_fFontX + m_fX, m_fFontY - m_fY), XMVectorSet((208.f / 255.f) * m_fAlpha, (177.f / 255.f) * m_fAlpha, (52.f / 255.f) * m_fAlpha, m_fAlpha));
...
HRESULT CDialogue_Choice::Ready_Components(void* pArg)
{
	...
	if (FAILED(Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Texture_Dialogue_Chice"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
	if (FAILED(Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Texture_UI_T_InteractSelectionBracket"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom1), nullptr)))
	{
		return E_FAIL;
	}
	if (FAILED(Add_Asset_Component(g_iStaticLevel, TEXT("Prototype_Texture_UI_T_Icon_InteractType_Straight"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom2), nullptr)))
	{
		return E_FAIL;
	}
