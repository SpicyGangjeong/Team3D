...
class CState_Spell final : public CState_Root
{
public:
    typedef struct tagSpell : public CState_Root::STATE_ROOT_DESC {
        function<void(_float fTimeDelta)> funcPriorityUpdate = { nullptr };
        function<void(_float fTimeDelta)> funcLateUpdate = { nullptr };
    }STATE_SPELL_DESC;
private:
    CState_Spell();
    virtual ~CState_Spell() = default;
...
private:
    HRESULT Initialize(STATE_SPELL_DESC* pDesc);

public:
    static CState_Spell* Create(STATE_SPELL_DESC* pDesc);
