...
class CState_Dash final : public CState_Root
{
public:
    typedef struct tagDash : public CState_Root::STATE_ROOT_DESC {
        function<void(_float fTimeDelta)> funcPriorityUpdate = { nullptr };
        function<void(_float fTimeDelta)> funcLateUpdate = { nullptr };
    }STATE_DASH_DESC;
private:
    CState_Dash();
    virtual ~CState_Dash() = default;
...
private:
    HRESULT Initialize(STATE_DASH_DESC* pDesc);

public:
    static CState_Dash* Create(STATE_DASH_DESC* pDesc);
