#pragma once
#include "Client_Define.h"
#include "State_Root.h"

NS_BEGIN(Client)

class CBroom_State_Fly final : public CState_Root
{
public:
    typedef struct tagFly : public CState_Root::STATE_ROOT_DESC {
        function<void(_float fTimeDelta)> funcPriorityUpdate = { nullptr };
        function<void(_float fTimeDelta)> funcLateUpdate = { nullptr };
    }STATE_FLY_DESC;
private:
    CBroom_State_Fly();
    virtual ~CBroom_State_Fly() = default;

public:
    virtual void Enter();
    virtual HRESULT Update(_float fTimeDelta);
    virtual void Exit();

private:
    function<void(_float fTimeDelta)> m_funcPriorityUpdate = { nullptr };
    function<void(_float fTimeDelta)> m_funcLateUpdate = { nullptr };

private:
    HRESULT Initialize(STATE_FLY_DESC* pDesc);

public:
    static CBroom_State_Fly* Create(STATE_FLY_DESC* pDesc);
    virtual void Free() override;
    virtual void Describe_Entity();
};

NS_END
