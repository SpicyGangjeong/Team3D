...
class CCallBack_NonPlayable_HitReport final : public PSX::PxUserControllerHitReport
{
	// 구현 예시 
...
private:
	CCallBack_NonPlayable_HitReport();
public:
	~CCallBack_NonPlayable_HitReport();
...
public:
	HRESULT Initialize(CCharacter_Controller* pController);
	HRESULT Finalize();
	_bool IsOnGround();
...
public:
	static CCallBack_NonPlayable_HitReport* Create();
};
NS_END
