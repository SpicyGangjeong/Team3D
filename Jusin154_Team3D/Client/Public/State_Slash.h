...
class CState_Slash final : public CState_Root
{
public:
    typedef struct tagSlash : public CState_Root::STATE_ROOT_DESC {
        function<void(_float fTimeDelta)> funcPriorityUpdate = { nullptr };
        function<void(_float fTimeDelta)> funcLateUpdate = { nullptr };
    }STATE_SLASH_DESC;
private:
    CState_Slash();
    virtual ~CState_Slash() = default;
...
private:
    HRESULT Initialize(STATE_SLASH_DESC* pDesc);

public:
    static CState_Slash* Create(STATE_SLASH_DESC* pDesc);
