...
PS_OUT PS_EYELASH_DAOTHV_ToSRO(PS_IN In)
{
    ...
    Out.vDepth = float4((In.vProjPos.z / In.vProjPos.w), // NDC 깊이 ( 0~ 1)
        ...
        AI_TEXTURE_TYPE_SPECULAR / AI_TEXTURE_TYPE_MAX, // 서페이스 파라미터
        1.f);
...
PS_OUT PS_UPPER_DMRON_ToSRO(PS_IN In)
{
    PS_OUT Out;
    ...
    Out.vDepth = float4((In.vProjPos.z / In.vProjPos.w), // NDC 깊이 ( 0~ 1)
        (In.vProjPos.w / g_fFar), // 뷰 스페이스 Z 
        AI_TEXTURE_TYPE_METALNESS / AI_TEXTURE_TYPE_MAX, // 서페이스 파라미터
        1.f);
    Out.vColor = float4(0.f, 0.f, 0.f, 1.f);
...
technique11 DefaultTechnique
{
    ...
    pass UPPER_DMRON_ToSRO // 8
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_UPPER_DMRON_ToSRO();
    }
    pass GLASSES_DMRON_ToSRO // 9
    {
        SetRasterizerState(RS_Default);
        SetDepthStencilState(DSS_Default, 0);
        SetBlendState(BS_None, float4(0.f, 0.f, 0.f, 0.f), 0xffffffff);
