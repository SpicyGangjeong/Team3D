#pragma once
#include "Editor_Define.h"
#include "State_Root.h"

class CState_Dodge final : public CState_Root
{
public:
    typedef struct tagDodge : public CState_Root::STATE_ROOT_DESC {
        function<void(_float fTimeDelta)> funcPriorityUpdate = { nullptr };
        function<void(_float fTimeDelta)> funcLateUpdate = { nullptr };
    }STATE_DODGE_DESC;
public:
    CState_Dodge();
    virtual ~CState_Dodge() = default;

public:
    virtual void Enter();
    virtual HRESULT Update(_float fTimeDelta);
    virtual void Exit();

private:
    function<void(_float fTimeDelta)> m_funcPriorityUpdate = { nullptr };
    function<void(_float fTimeDelta)> m_funcLateUpdate = { nullptr };

private:
    HRESULT Initialize(STATE_DODGE_DESC* pDesc);

public:
    static CState_Dodge* Create(STATE_DODGE_DESC* pDesc);
    virtual void Free() override;
    virtual void Describe_Entity();
};
