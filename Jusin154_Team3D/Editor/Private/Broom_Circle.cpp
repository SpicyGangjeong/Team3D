...
void CBroom_Circle::Update(_float fTimeDelta)
{
	...
	if (m_pGameInstance->Key_Down(DIK_K))
	{
		...
		m_fDissolve = 0.f;
		m_fFontAlpha = 1.f;
		static_cast<CUIObject*>(m_pOwner)->Function_Callback(TEXT("BroomStart"));
	}

	if (m_bTimer == true && m_iCount >= 0)
	{
		m_fTime += fTimeDelta;
		if (m_fTime >= 1.f)
		{
...
HRESULT CBroom_Circle::Ready_Components(void* pArg)
{
	...
	if (FAILED(Add_Asset_Component(ENUM_CLASS(LEVEL::UI), TEXT("Prototype_Texture_UI_T_AccioBallFrame_4K"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
