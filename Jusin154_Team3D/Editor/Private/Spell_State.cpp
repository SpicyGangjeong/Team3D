...
HRESULT CSpell_State::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CSpell_State::Initialize(void* pArg)
{
...
void CSpell_State::Priority_Update(_float fTimeDelta)
{
...
void CSpell_State::Update(_float fTimeDelta)
{
	if (!__super::Chack_Visible())
	{
		return;
...
void CSpell_State::Late_Update(_float fTimeDelta)
{
	if (!__super::Chack_Visible())
	{
		return;
...
_vector CSpell_State::Get_WorldPostion()
{
	return m_pTransformCom->Get_State(STATE::POSITION);
}

void CSpell_State::SizeUpX(_float fSizeX)
{
	m_fSizeX = fSizeX;
	m_pVIBufferCom->Set_SizeX(m_fSizeX);
}

void CSpell_State::SizeUpY(_float fSizeY)
{
	m_fSizeY = fSizeY;
	m_pVIBufferCom->Set_SizeY(m_fSizeY);
}

void CSpell_State::SizeUpdate(_float fSizeX, _float fSizeY)
{
	m_fSizeX = fSizeX;
	m_fSizeY = fSizeY;
	m_pVIBufferCom->Set_Size(m_fSizeX, m_fSizeY);
...
HRESULT CSpell_State::Ready_Components(void* pArg)
{
	...
	if (FAILED(Add_Asset_Component(ENUM_CLASS(LEVEL::UI), TEXT("Prototype_Texture_UI_T_NewLock"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
...
CSpell_State* CSpell_State::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CSpell_State* pInstance = new CSpell_State(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CSpell_State");
...
CGameObject* CSpell_State::Clone(void* pArg, CGameObject* pOwner)
{
	CSpell_State* pInstance = new CSpell_State(*this);
	pInstance->m_pOwner = pOwner;
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CSpell_State");
		SAFE_RELEASE(pInstance);
	}
...
void CSpell_State::Describe_Entity()
{
}
