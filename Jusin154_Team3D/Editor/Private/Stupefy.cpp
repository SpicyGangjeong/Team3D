...
HRESULT CStupefy::Initialize(void* pArg)
{
	...
	m_pProjectile = Get_PartObject<CEditEffect>("Stupefy_PJ");
	SAFE_ADDREF(m_pProjectile);

	m_pProjectile2 = Get_PartObject<CEditEffect>("Stupefy_PJ2");
	SAFE_ADDREF(m_pProjectile2);


	m_pStupefy_PJ_PT = Get_PartObject<CEditEffect>("Stupefy_PJ_PT");
...
HRESULT CStupefy::Pre_Setting(CGameObject* pObject, void* pArg)
{
	...
	CEditEffect* pWand_Distortion = Get_PartObject<CEditEffect>("Wand_Distortion");

	CEditEffect* pStupefy_Light_Long = Get_PartObject<CEditEffect>("Stupefy_Light_Long");
	CEditEffect* pBodyFlare = Get_PartObject<CEditEffect>("BodyFlare");

	CEditEffect* pStupefy_Smoke = Get_PartObject<CEditEffect>("Stupefy_Smoke");
...
void CStupefy::OnCollision(CGameObject* pOther, void* pDesc)
{
	...
	CEditEffect* pStupefy_Hit_PT = Get_PartObject<CEditEffect>("Stupefy_Hit_PT");
	CEditEffect* pStupefy_Circle0 = Get_PartObject<CEditEffect>("Stupefy_Circle0");
	CEditEffect* pHitFlare = Get_PartObject<CEditEffect>("HitFlare");
	...
	m_pStupefy_PJ_PT->Get_Component<CTransform>()->Set_State(STATE::POSITION, XMVectorSet(0.f, -9999.f, 0.f, 1.f)); 
