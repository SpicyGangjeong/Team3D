...
HRESULT CBroom::Ready_Child()
{
	...
	if (FAILED(Add_PartObject<CEditEffect>("Wind_Screen", NEXT_LEVEL, &m_pWindEffect, &PartsDesc)))
	{
		return E_FAIL;
	}
	...
	if (FAILED(Add_PartObject<CEditEffect>("Boost_Screen", NEXT_LEVEL, &m_pBoostScreenFX, &PartsDesc)))
	{
		return E_FAIL;
	}
	m_pBoostScreenFX->Load("../Bin/Resources/Data/Effect/BroomEffect/Booster_ScreenFX", static_cast<LEVEL>(NEXT_LEVEL));
	

	///

	if (FAILED(Add_PartObject<CTrailObject>("Broom_Trail", NEXT_LEVEL, &m_pBroomTrail, &PartsDesc))) {
		return E_FAIL;
	}

	m_pBroomTrail->Load_Trail("../Bin/Resources/Data/Effect/BroomEffect/Broom_Trail", static_cast<LEVEL>(NEXT_LEVEL));
