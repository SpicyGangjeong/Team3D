...
HRESULT CQuest_Slot::Initialize(void* pArg)
{
	...
	m_iQuestCount = static_cast<CQuest_Panel*>(m_pOwner)->Set_Quest_Count(ENUM_CLASS(QUESTSTATE::NONE));
	Visible(true);
	return S_OK;
}
...
HRESULT CQuest_Slot::Render()
{
	...
	for (_int i = 0; i < m_iQuestCount; ++i)
	{
		...
		if (i == m_iQuestIndex)
		{
			m_pGameInstance->Render_Text(TEXT("Font_size30"), static_cast<CQuest_Panel*>(m_pOwner)->Get_Quest(i).pQuestName.c_str(), _float2(m_fFontX - 10.f + m_fX, m_fFontY - 10.F - m_fY));
		}
		else
		{
			m_pGameInstance->Render_Text(TEXT("Font_size20"), static_cast<CQuest_Panel*>(m_pOwner)->Get_Quest(i).pQuestName.c_str(), _float2(m_fFontX + m_fX, m_fFontY - m_fY));
		}
	}
	return S_OK;
}
...
HRESULT CQuest_Slot::Ready_Components(void* pArg)
{
	...
	if (FAILED(Add_Asset_Component(ENUM_CLASS(LEVEL::UI), TEXT("Prototype_Texture_UI_T_MenuTextButtonBorder"), reinterpret_cast<CComponent**>(&m_pDiffuse_TextureCom), nullptr)))
	{
		return E_FAIL;
	}
...
void CQuest_Slot::Set_QuestType(_int Index)
{
	...
	case 1:
		m_iQuestCount = static_cast<CQuest_Panel*>(m_pOwner)->Set_Quest_Count(ENUM_CLASS(QUESTSTATE::ACCEPTED));
		m_pVIBufferCom->Set_Draw(m_iQuestCount);
		break;

	case 2:
		m_iQuestCount = static_cast<CQuest_Panel*>(m_pOwner)->Set_Quest_Count(ENUM_CLASS(QUESTSTATE::CLEARED));
		m_pVIBufferCom->Set_Draw(m_iQuestCount);
		break;
