...
HRESULT CRanrok_Prop::Initialize(void* pArg)
{
	...
	m_pSphere = Get_PartObject<CEditEffect>("Sphere");
	m_pSphereLay = Get_PartObject<CEditEffect>("Sphere_Noise");
	m_pSphereHitLay = Get_PartObject<CEditEffect>("HitLay");

	m_pRing = Get_PartObject<CEditEffect>("Ring");
...
void CRanrok_Prop::Update(_float fTimeDelta)
{
	...
	if (m_fSizeAccTime > XM_PIDIV2 && m_isSizeLerpEnd == false)
	{
		...
		CEditEffect* pGlowBall = Get_PartObject<CEditEffect>("GlowBall");
		CEditEffect* pCore = Get_PartObject<CEditEffect>("Core");
...
HRESULT CRanrok_Prop::Pre_Setting(CGameObject* pObject, void* pArg)
{
	...
	_vector vRandomPos = XMVectorSet(m_pGameInstance->Real_Random_Float(-10.f, 10.f), m_pGameInstance->Real_Random_Float(5.f, 10.f), m_pGameInstance->Real_Random_Float(-10.f, 10.f), 0.f);
	...
	CEditEffect* pAppear_PT = Get_PartObject<CEditEffect>("Appear_PT");
	CEditEffect* pSpline = Get_PartObject<CEditEffect>("Spline");
...
void CRanrok_Prop::OnCollision(CGameObject* pOther, void* pDesc)
{
	...
	if (m_fHp > 0)
	{
		CEditEffect* pHitLay = Get_PartObject<CEditEffect>("HitLay");
		CEditEffect* pHit_PT = Get_PartObject<CEditEffect>("Hit_PT");
		CEditEffect* pPulse_Purple = Get_PartObject<CEditEffect>("Pulse_Purple");
	...
	else
	{
		...
		CEditEffect* pHitSplash0 = Get_PartObject<CEditEffect>("HitSplash0");
		CEditEffect* pHitSplash1 = Get_PartObject<CEditEffect>("HitSplash1");
		CEditEffect* pBroken_Goo = Get_PartObject<CEditEffect>("Broken_Goo");
		CEditEffect* pBroken_PT = Get_PartObject<CEditEffect>("Broken_PT");
		CEditEffect* pBroken_PT2 = Get_PartObject<CEditEffect>("Broken_PT2");
