...
void CCharacter_Controller::Move(_float fTimeDelta)
{
	...
	if (true == m_bGravity) {
		m_pTransform->AccumulateMomentum(XMVectorSet(0.f, -3.5f * fTimeDelta, 0.f, 0.f));
	}
...
void CCharacter_Controller::Describe_Entity()
{
	...
	GUI::Text("%.1f", cosf(m_fSlopeLimit)); GUI::SameLine(); GUI::SliderFloat("m_fSlopeLimit", &m_fSlopeLimit, 0.0f, 1.f);
