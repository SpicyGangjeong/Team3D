...
// F->D
float NDF_ggxtr(float3 n, float3 h, float a) // NormalDistributionGGXTR, (H, halfWay vector), (A, Roughness), (N, Normal?)
{
    float a2 = a * a;
...
PBR_LIGHT_OUT PBR_Lighting(
    ...
    float3 vLightColor, float fAttenuation
    ...
    float3 F0 = lerp(float3(0.04f, 0.04f, 0.04f), vAlbedo, fMetallic);
    
    float D = NDF_ggxtr(vNormal, vHalfVector, fAlpha);
    float G = Geometry_Smith(vNormal, vToView, vToLight, k);
    float3 F = Fresnel_Schlick(saturate(dot(vHalfVector, vToView)), F0);
